...
def get_list_total_reads(directory_path, contig):
...
def get_pileup(file_path, contig):
    global contig_num_reads_dict
    ...
    sample_name = re.search("(ENCSR[0-9A-Za-z_]*)_filtered", file_path).group(1)

    #Not sure what this does entirely?
    pos_depth_list = pd.DataFrame([*samfile.count_coverage(contig, 0, 1870, quality_threshold=0)]).sum().T
